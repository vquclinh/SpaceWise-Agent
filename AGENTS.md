...
- **Change:** the Phase 1 `facilities` catalogue table is **dropped**. Its only substantive attribute, `facility_name`, is carried forward as a checked attribute on the Phase 2 facility-bearing tables. The interim design that absorbed it into `space_facilities` was superseded by the documented `space_facilities` drop in §1b. `facility_id` and `facilities.description` have no replacement column and are dropped as data.
...
- **Design tradeoff (recorded for the record):** `facility_name` is no longer FK-validated against a catalogue; it is instead constrained by synchronized `CHECK` whitelists on `facility_assets` and `space_facility_requirements` (matching the convention already used for `role`, `space_type`, `status`, etc. in this schema, rather than a lookup table). Adding a new facility type now requires a migration to extend those `CHECK` lists, the same tradeoff every other enum column in this schema already carries.
- **Downstream impact:** current `outputs/10-schema-migration-G08.sql` implements this amendment by archiving `facilities`, expanding legacy facility data before the drop, and validating that the table no longer exists after migration.
...
- **Downstream impact:** current `outputs/10-schema-migration-G08.sql` implements this amendment by archiving and dropping `space_facilities`, expanding legacy quantities into `facility_assets`, and validating that no `space_facilities` table remains.
...
- **Downstream impact:** current `outputs/10-schema-migration-G08.sql` implements this amendment by seeding `user_roles` from the legacy `user_accounts.role` values, verifying row counts, dropping the role check constraint, and then dropping the column.
...
- **Working commands now:** `/audit-smoke-test` (safe rehearsal), the completed Phase 1 generation commands `/01-generate-business-req` … `/07-generate-query-design`, and the Phase 2 commands currently present under `.opencode/commands/` (`/11-concurrency-design` through `/16-analytical-queries`).
- **Phase 2 command/skill status:** task-specific skills exist for Tasks 08–16 under `.opencode/skills/db-design-pipeline/`. Command files are present for Tasks 11–16; command files for Tasks 08–10 are not present, so do not invoke absent commands. When a task owner adds or changes a command/skill, they must follow the repository audit policy (section 8).
